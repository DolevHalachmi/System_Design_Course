...
struct names_and_events {
    ...
    // Default constructor for -Weffc++
    names_and_events() : team_a_name(""), team_b_name(""), events() {}

    // Parameterized constructor to allow the initialization in event.cpp
    names_and_events(std::string a, std::string b, std::vector<Event> e)
        : team_a_name(std::move(a)), 
          team_b_name(std::move(b)), 
          events(std::move(e)) 
    {}
};
